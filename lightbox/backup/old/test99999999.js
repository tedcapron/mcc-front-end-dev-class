...
function picselect(e){
 for ( ty=0 ; ty<12; ty++ ) {
 const mm=document.getElementById(`alt${ty}`).addEventListener("click", function(e){
e.preventDefault();
 
// pickpic=e.target.href;
// // modview(pickpic);
// console.log(oc);
 });
};
...
function modview(j) {
  ...
  md.style.backgroundImage = `url(${pickpic})`;

  // md.style.backgroundImage = `url(${path})`;
  
  md.setAttribute("class", "modalView");
  document.getElementById("modal").src = j;
